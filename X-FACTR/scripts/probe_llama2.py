...
LM_NAME = {
    ...
    'llama2': 'meta-llama/Llama-2-7b-chat-hf',# # meta-llama/Llama-2-70b-chat-hf #meta-llama/Llama-2-7b-chat-hf
            ...
            try:
                ...
                with CsvLogFileContext(log_filename, headers=headers) as csv_file, \
                    ...
                    for prompt in prompts:
                        ...
                        if self.args.log_acc is not None:
                            acc_dict = {'relation':relation, 'acc':np.mean(acc), 'acc_ori':np.mean(acc_ori), 'len_acc':np.mean(len_acc), 'len_acc_ori': np.mean(len_acc_ori), 'prompt':prompt}
                            ...
                            if os.path.exists(path):
                                # append the next PID
                                acc_df.to_csv(f'{path}/{self.args.lang}.csv', mode='a', index=False, header=False)
...
if __name__ == '__main__':
    ...
    if args.model == 'llama2':
        print('using llama2')
        print('LM: ', LM)
        #LM = 'daryl149/llama-2-70b-chat-hf'
        token = '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'
        
        print('load data')
        tokenizer = AutoTokenizer.from_pretrained(LM, use_auth_token=token)

        print('load model')
        model = AutoModel.from_pretrained(LM, use_auth_token=token)
    else:
        # load data
        print('load data')
